...
function addBeforeEnter(router) {
  router.beforeEach((to, from, next) => {
    ...
    // User is authenticated and trying to access a path with no special permission
      ...
      store.state.auth.isAuthorize === true &&
      to.meta.authLevel <= Roles.Level.Default
    ) {
      next()
    }
    // User is authenticated but is trying to access a path beyond their access level
      ...
      store.state.auth.isAuthorize === true &&
      to.meta.authLevel > level
    ) {
      ConMsgs.methods.$_console_log(
        '[Router] beforeEach: DENIED. User is not authorized or does not have sufficient access level'
      )
      next('/home/start')
    }
    // Unauthorized path. User doesn't have access to this module
      ...
      ConMsgs.methods.$_console_log(
        '[Router] beforeEach: DENIED. User does not have access to this module'
      )
      next(false)
    } else {
      next()
