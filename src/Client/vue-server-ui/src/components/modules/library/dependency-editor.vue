          ...
          <v-flex xs12 sm6>
            <v-list-item-group v-model="activeObject">
              <v-list-item v-for="(item, index) in activeList" :key="index">
                ...
                <v-list-item-content
                  v-else-if="activeType === typeConsts.Bookcase"
                >
                  {{ item.name }}
                </v-list-item-content>
                <v-list-item-content
                  v-else-if="activeType === typeConsts.Series"
                >
                  {{ item.name }}
                </v-list-item-content>
                <v-list-item-content
                  v-else-if="activeType === typeConsts.Shelf"
                >
                  {{ item.name }}
                </v-list-item-content>
              </v-list-item>
            ...
            >
              ...
              <v-form v-model="validated" ref="objectForm" lazy-validation>
                <template v-if="activeType === typeConsts.Author">
                  <v-flex xs12 pl-2>
                    <v-text-field
                      v-model="selectedObject.firstName"
                      label="First Name"
                    ></v-text-field>
                  </v-flex>
                  <v-flex xs12 pl-2>
                    <v-text-field
                      v-model="selectedObject.lastName"
                      label="Last Name"
                  ...
                  <v-flex xs12 pl-1>
                    <v-checkbox
                      v-model="selectedObject.deceased"
                      label="Deceased"
                    ></v-checkbox>
                  </v-flex>
                </template>
                ...
                <template v-else-if="activeType === typeConsts.Shelf">
                  <v-flex xs12 pl-2>
                    <v-select
                      v-model="selectedObject.bookcaseId"
                      :items="bookcaseList"
                      label="Bookcase"
                      item-value="id"
                      item-text="name"
              ...
              <!-- Generic to all -->
              <v-flex xs12>
                <v-btn @click="selectedObject = null"> CANCEL </v-btn>
                <v-btn
                  class="primary"
                  @click="updateObject()"
                  :disabled="!validated"
...
export default {
  ...
  data() {
    return {
      ...
      activeType: null,
      activeList: [],
      typeList: [AUTHOR_LABEL, BOOKCASE_LABEL, SERIES_LABEL, SHELF_LABEL],
      typeConsts: {
        Author: AUTHOR_VALUE,
  ...
  watch: {
    ...
    dialogOpen(newValue) {
      if (newValue === false) {
        this.$_console_log(
          '[Dependency Editor Dialog Open] Dialog open = false'
        )
        this.$emit('closeDialog', false)
      } else {
        this.$_console_log(
          '[Dependency Editor Dialog OpenAuthor Dialog] Dialog open = true'
        )
      }
    },
    activeType(newValue, oldValue) {
      this.$_console_log(
        `[Dependency Editor Active Type] New value is: ${newValue}`
      )
  ...
  methods: {
    ...
    addObject() {
      if (this.activeType === null || typeof this.activeType !== 'number') {
        this.$_console_log(
          `[Dependency Editor] Add Object: Active type is null or not a number: (${this.activeType})`
        )
        return
      }
      ...
      if (obj === null) {
        this.$_console_log(
          `[Dependency Editor] Add Object: New object is null. Not making a new object`
        )
        return
      }
    ...
    updateObject() {
      if (this.activeType === null || typeof this.activeType !== 'number') {
        this.$_console_log(
          `[Dependency Editor] Update Object: Active type is null or not a number: (${this.activeType})`
        )
        return
      }
      ...
      if (this.selectedObject.id === 0) {
        ...
        if (this.$refs.objectForm.validate()) {
          this.$store.dispatch(
            `edit${this.typeList[this.activeType]}`,
            this.selectedObject
          )
          this.getActiveList()
        }
    ...
    deleteObject() {
      if (this.activeType === null || typeof this.activeType !== 'number') {
        this.$_console_log(
          `[Dependency Editor] Delete Object: Active type is null or not a number: (${this.activeType})`
        )
        return
      }
