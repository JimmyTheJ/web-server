    ...
    <v-sheet class="text-center scrolling-y" height="160px">
      ...
      >
        <div :class="[getMessageBackground(message.read), 'my-1']">
          ...
          <v-banner>
            <v-avatar
              v-if="messageHasGroupNum(message.group)"
              color="orange"
              size="40"
            >
            ...
            <span
              ...
              :class="getTextColor(message.read)"
              >{{ message.text }}</span
            >
...
export default {
  ...
  methods: {
    deleteMessage(item) {
      this.$_console_log(
        `[Notification Bar] Delete Message with id: ${item.id}`
      )
    ...
    getTextColor(isRead) {
      if (this.$vuetify.theme.dark === true) {
        if (isRead) return 'white--text'
        else return 'white--text font-weight-bold'
      } else {
        if (isRead) return 'black--text'
        else return 'black--text font-weight-bold'
      }
    },
    ...
    getMessageBackground(isRead) {
      if (this.$vuetify.theme.dark === true) {
        if (isRead) return 'grey darken-3'
        else return 'grey'
      } else {
        if (isRead) return 'grey lighten-3'
        else return 'grey'
      }
    },
